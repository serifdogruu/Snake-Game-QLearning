...
#from Modified_Game import SnakeGameAI, Direction, Point
from denememe import SnakeGameAI, Direction, Point
...
class Agent :
    ...
    def remember(self, state, action, reward, next_state, done):
        self.memory.append((state, action, reward, next_state, done)) #popleft is max_mem reached
...
def train():
    ...
    while True :
        ...
        if done :
            # train long memory (call : replay memory or experience replay)
            #plot the results
            game.reset()
            agent.n_games += 1
